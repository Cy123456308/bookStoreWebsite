        ...
        success: function (data) {
            if (data != null) {
                ...
                $.each(data, function (i, item) {
                    if (isSelect) {
                        // 如果是select元素，生成option
                        var selected = (item["Id"] == stateProvinceId) ? 'selected' : '';
        ...
        success: function (data) {
            if (data != null) {
                ...
                $.each(data, function (i, item) {
                    if (isSelect) {
                        // 如果是select元素，生成option
                        var selected = (item["Id"] == cityId) ? 'selected' : '';
                        obj.append('<option value="' + item["Id"] + '" ' + selected + '>' + item["Name"] + '</option>');
                    } else if (type == "mobile") {
                        // 移动端样式 - li元素
                        var currentClass = (item["Id"] == cityId) ? ' current' : '';
                        obj.append('<li class="mw-address-item' + currentClass + '" cityId="' + item["Id"] + '" name="' + item["Name"] + '"><span class="mw-iconfont">&#xa018;</span><div>' + item["Name"] + '</div></li>');
                    } else {
                        // PC端样式 - li元素
                        var currentClass = (item["Id"] == cityId) ? ' w-address-current' : '';
                        obj.append('<li class="w-address-conul-item' + currentClass + '" cityId="' + item["Id"] + '"><a class="w-address-conul-link">' + item["Name"] + '</a></li>');
                    }
                });
            }
        },
...
// 主页面代码 - 处理内联样式背景图 + 注册Service Worker处理图片请求
(function () {
    const DOMAINS_MAP = {
    ...
    function handleImageError(event) {
        ...
        // 避免无限循环：如果当前 src 已经是备用 URL 且仍失败，则放弃
        ...
        // 替换为备用域名
        img.src = fallbackSrc;
        // 更新标记，表示已经使用过 fallback
        img.setAttribute('data-fallback-used', 'true');
    }

    function processImages() {
        document.querySelectorAll('img:not([data-img-processed])').forEach(img => {
            const originalSrc = img.src;
            // 只处理需要域名替换的图片
            if (!DOMAIN_REGEX.test(originalSrc)) {
                img.setAttribute('data-img-processed', 'true');
                return;
            }

            const fallbackSrc = originalSrc.replace(DOMAIN_REGEX, (match) => {
                return DOMAINS_MAP[match.toLowerCase()] || match;
            });

            // 存储原始和备用 URL
            img.setAttribute('data-original-src', originalSrc);
            img.setAttribute('data-fallback-src', fallbackSrc);
            // 移除已有的 error 监听，避免重复绑定
            img.removeEventListener('error', handleImageError);
            img.addEventListener('error', handleImageError);

            // 确保使用原始 URL（如果当前 src 已经被其他逻辑改过，则还原）
    ...
    // ===== 2. 处理内联样式背景图（预检测，失败后替换）=====
    async function processInlineBackgrounds() {
        const elements = document.querySelectorAll('[style*="background"]:not([data-bg-processed])');
        if (!elements.length) return;

        // 使用 Promise.all 并行检测所有元素的背景图，避免阻塞
        const tasks = Array.from(elements).map(async (element) => {
            ...
            // 收集需要检测的原始 URL 及其对应备用 URL
            const checks = [];
            urlMatches.forEach(fullMatch => {
                const urlMatch = fullMatch.match(/url\(["']?([^"')]+)["']?\)/);
                if (!urlMatch) return;
                const originalUrl = urlMatch[1];
                if (!DOMAIN_REGEX.test(originalUrl)) return;

                const fallbackUrl = originalUrl.replace(DOMAIN_REGEX, (match) => {
                    return DOMAINS_MAP[match.toLowerCase()] || match;
                });

                checks.push({ originalUrl, fallbackUrl, fullMatch });
            });
            ...
            const results = await Promise.allSettled(
                checks.map(({ originalUrl }) => {
                    return new Promise((resolve) => {
                        const testImg = new Image();
                        testImg.onload = () => resolve(true);  // 成功
                        testImg.onerror = () => resolve(false); // 失败
            ...
            checks.forEach(({ originalUrl, fallbackUrl, fullMatch }, idx) => {
                const success = results[idx].value === true;
                if (!success) {
                    // 原始图片加载失败，替换 URL
                    newStyle = newStyle.replace(originalUrl, fallbackUrl);
                    hasReplacement = true;
                }
            });
    ...
    function setupMutationObserver() {
        const observer = new MutationObserver((mutations) => {
            let needsProcessImages = false;
            let needsProcessBg = false;

            mutations.forEach(mutation => {
                // 新增节点
                if (mutation.type === 'childList' && mutation.addedNodes.length) {
                    needsProcessImages = true;
                    needsProcessBg = true;
                }
                ...
                if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                    ...
                    if (element.hasAttribute('style')) {
                        element.removeAttribute('data-bg-processed');
                        needsProcessBg = true;
                    }
                }
                ...
                if (mutation.type === 'attributes' && mutation.attributeName === 'src') {
                    ...
                    if (element.tagName === 'IMG') {
                        element.removeAttribute('data-img-processed');
                        needsProcessImages = true;
                    }
                }
            });
            ...
            // 延迟处理，避免频繁触发
            if (needsProcessImages) {
                setTimeout(processImages, 50);
            }
            if (needsProcessBg) {
                setTimeout(processInlineBackgrounds, 50);
            }
        });
